...
class Pool extends Base
{
    ...
    {
        return $this->redis->zcount($this->name, '-inf', $this->getTime());
    }
    ...
    public function addItem($item)
    {
        ...
        $this->redis->zadd($this->name, $this->getTime(), $item);
    ...
    public function getItems($size)
    {
        ...
        foreach ($steps as $size) {
            $chunk = $this->redis->poolGet($this->name, $size, $this->getTime(), $this->options[self::OPT_ACK_TTL]);
        ...
        foreach (array_chunk($items, $this->options[self::OPT_ACK_MAX_CHUNK_SIZE], true) as $chunk) {
            try {
                ...
                foreach ($chunk as $item) {
                    $pipe->poolAck($this->name, $item, $this->getTime() + $this->options[self::OPT_ACK_VALID_FOR]);
                }
                $pipe->execute();
                $this->waitForSlaveSync();
    ...
    {
        $this->redis->poolAck($this->name, $item, $this->getTime() + $this->options[self::OPT_ACK_VALID_FOR]);
    }
